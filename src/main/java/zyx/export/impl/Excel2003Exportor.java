...
public class Excel2003Exportor implements IFileExportor {
    @Override
    public Workbook getExportResult(List<?> data, List<ExportCell> exportCells)
            throws FileExportException {
        // XSSFWorkbook used for .xslx (>=2007), HSSFWorkbook for 03 .xls
        Workbook workbook = new HSSFWorkbook();// 创建 一个excel文档对象
        Sheet sheet = workbook.createSheet();// 创建一个工作薄对象
        Row titleRow = sheet.createRow(0);// 创建一个行对象,从0行开始
    ...
    private Workbook createContentRowsByMap(Workbook workbook, List<Map> dataList, List<ExportCell> exportCells, Sheet sheet)
            throws FileExportException {
        if (!dataList.isEmpty()) {
            int rowNum = 1;
    ...
    private static void createContentRowsByBean(
            Workbook workbook, List<Object> dataList, List<ExportCell> exportCells, Sheet sheet)
            throws FileExportException {
        int rowNum = 1;
        if (!dataList.isEmpty()) {
            CellStyle cellStyle = createCellStyle(workbook);
            for (Object t : dataList) {
                ...
                for (int colNum = 0; colNum < exportCells.size(); colNum++) {
                    ...
                    Object obj = null;
                    try {
                        obj = ReflectionUtils.executeMethod(t, ReflectionUtils.returnGetMethodName(
                                exportCell.getAlias()));
                    } catch (Exception e) {
                        throw new FileExportException(
                                "执行executeMethod  出错 Alias is " + exportCell.getAlias()
                                + " at " + e.getMessage());
                    }
                    setCellValue(obj, cell);
                }
